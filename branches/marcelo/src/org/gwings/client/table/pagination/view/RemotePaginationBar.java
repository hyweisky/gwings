...
import org.gwings.client.table.pagination.model.AsyncDataProvider;
...
public class RemotePaginationBar<T extends Plotable> extends Composite implements RemotePagerListener<T> {
    ...
    private RemotePager<T> pager;
    private BusyWidget busy;

    public RemotePaginationBar(PaginatedScrollTable<T> table){
        setTable(table);
        init();
        setupLayout();
        setupListeners();
     ...
     */
    public void setPager(RemotePager<T> pager) {
        if(this.pager != null){
            this.pager.removeRemotePagerListener(this);
        }
        
        this.pager = pager;
        this.pager.addRemotePagerListener(this);
    }
    ...
    private void mountRange() {
        ...
        try {
            totalPages = pager.getPageConfig().getTotalPages();
            if(totalPages == 1){
     ...
     */
    public AsyncDataProvider<T> getProvider() {
        return pager.getAsyncProvider();
    }
     ...
     */
    public void setProvider(AsyncDataProvider<T> provider) {
//        pager.setProvider(provider);
    ...
    public void firstPageRequested(RemotePagerEvent<T> evt) {
        ...
        setPreviousDisabled(true);
        try {
            Integer totalPages = pager.getPageConfig().getTotalPages();
    ...
    public void pageWillChange(RemotePagerEvent<T> evt) {
        RemotePager<T> pager = evt.getPager();
        
        Integer index = pager.getPageConfig().currentPageIndex();
    ...
    public void firstPageArrived(RemotePagerEvent<T> evt) {
        System.out.println("primeira página chegou");
    }
